...
class Parser(pdlparser.PDLParser):
    ...
    def endPage(self, nextpos):
        ...
        pos3 = nextpos - 3
        minfile = self.minfile
        if minfile[pos3:nextpos-1].decode("utf-8", "replace") == \
            self.setNumberOfCopies.decode('utf-8', 'replace'):
    ...
    def setColorSpace(self, nextpos):
        ...
        if self.minfile[nextpos-4:nextpos-1].decode("utf-8", "replace") == self.RGBColorSpace: # TODO: doesn't seem to handle all cases !
            self.iscolor = True
        return 0
    ...
    def skipHPPCLXL(self, nextpos):
        ...
        minfile = self.minfile
        if nextpos \
           and ((minfile[nextpos:nextpos+11].decode("utf-8") == " HP-PCL XL;") \
             or (minfile[nextpos:nextpos+14].decode("utf-8") == " BROTHER XLHB;")):
            pos = nextpos
            while chr(minfile[pos]) != '\n':
                pos += 1
    ...
    def escape(self, nextpos):
        ...
        pos = endpos = nextpos
        minfile = self.minfile
        if minfile[pos: pos+8].decode('utf-8') == r"%-12345X":
            endpos = pos + 9
            endmark = chr(0x0c) + chr(0x00) + chr(0x1b)
            asciilimit = chr(0x80)
            quotes = 0

            while (chr(minfile[endpos]) not in endmark) \
                and ((chr(minfile[endpos]) < asciilimit) or (quotes % 2)):

                if chr(minfile[endpos]) == '"':
                    quotes += 1
                endpos += 1
            ...
            stuff.append(minfile[pos: endpos].decode('utf-8', 'replace'))
    ...
    def skipKyoceraPrescribe(self, nextpos):
        ...
        pos = nextpos - 1
        minfile = self.minfile
        if minfile[pos:pos+3].decode("utf-8") == "!R!":
            while (pos - nextpos) < 1024:   # This is a realistic upper bound, to avoid infinite loops
                if (chr(minfile[pos]) == ";") and ((minfile[pos-4:pos].decode('utf-8')) == "EXIT"):
                    pos += 1
                    prescribe = self.prescribeStuff.setdefault(self.pagecount, [])
                    prescribe.append(minfile[nextpos-1:pos].decode("utf-8"))
    ...
    def getJobSize(self):
        ...
        self.setNumberOfCopies = b'\xf8\x31'
